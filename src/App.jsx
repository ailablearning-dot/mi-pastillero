...
function LoginScreen(){
  ...
  const handleGoogle=async()=>{
    await supabase.auth.signInWithOAuth({provider:"google",options:{redirectTo:"https://mi-pastillero.vercel.app"}});
  };
    ...
    <div style={{fontFamily:"'Nunito', sans-serif"}} className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-stone-100 flex items-center justify-center px-4">
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-3xl shadow-sm p-6">
          ...
          {msg&&<div className={`text-xs font-medium px-3 py-2 rounded-xl mb-3 ${msg.type==="error"?"bg-red-50 text-red-600":"bg-emerald-50 text-emerald-600"}`}>{msg.text}</div>}
          ...
          <div className="flex items-center gap-3 mb-3"><div className="flex-1 h-px bg-gray-200"/><span className="text-xs text-gray-400">o</span><div className="flex-1 h-px bg-gray-200"/></div>
                ...
                return(
                  <div key={pill.id} className={`flex items-center gap-3 p-4 rounded-2xl ${c.bg}`}>
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-400">{pill.dosis&&`${pill.dosis} · `}{pill.frecuencia}</p>
                    </div>
...
function SettingsScreen({session, pills, onUpdate, onBack}){
  ...
  const removePill=async(id)=>{
    await supabase.from("pastillas").delete().eq("id",id);
    const nl=list.filter(p=>p.id!==id);setList(nl);onUpdate(nl);
  };
                  ...
                  <button key={pill.id} onClick={()=>togglePill(todayStr,pill.id)}
                    ...
                    <div className="flex-1 text-left">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        {taken?`Tomada a las ${fmtTime(taken.time)}`:`${pill.dosis?pill.dosis+" · ":""}${pill.frecuencia||"Toca para registrar"}`}
                      </p>
                    </div>
