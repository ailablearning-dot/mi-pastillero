...
3. ~~**Correo de soporte**~~ ✅ HECHO (2026-08-23). `soporte@pastillero.jimbera.com`, vía **ImprovMX** gratis: MX 10/20 a `mx1|mx2.improvmx.com` + TXT `v=spf1 include:spf.improvmx.com ~all`, en host `pastillero` de los Custom records del DNS de Squarespace. Reenvía a `ailab.learning@gmail.com`; probado de extremo a extremo.
   ...
   - ✅ **Y también contesta** (2026-08-23). ImprovMX no guarda buzón, solo reenvía, así que por sí solo dejaba las respuestas saliendo desde `ailab.learning@gmail.com` — el remitente del que se huía. Resuelto con «Enviar como» en Gmail apuntando al **SMTP de Resend** (`smtp.resend.com:465` SSL, usuario literal `resend`, contraseña = API key), que ya tenía el dominio verificado. **No hizo falta tocar el DNS ni pagar ImprovMX**, cuyo envío es de su plan de pago.
     - ⚠️ **El ajuste que hay que marcar a mano:** Gmail → Cuentas e importación → «Al responder a un mensaje: **responder desde la misma dirección a la que se envió**». Viene desmarcado, y sin él queda el alias dado de alta pero contestando desde el Gmail igual. Falla en silencio.
     - Solo cambia las RESPUESTAS a lo que llegó a `soporte@`. Lo personal de esa cuenta sigue saliendo como siempre; para iniciar un correo como soporte hay que elegir la dirección en el desplegable «De».
     - ⚠️ Gmail rellena el formulario solo, mirando el MX del dominio, y **propone `mx1.improvmx.com:587` con usuario `soporte`: los cuatro campos mal**. `mx1` es el servidor que recibe, no uno de salida.
     - Probado de extremo a extremo el 2026-08-23: correo de `mmontero@mununo.com` → `soporte@` → reenvío a Gmail → respuesta → llegó con remitente `Mi Pastillero <soporte@pastillero.jimbera.com>` **a la bandeja de entrada, no a spam**.
