...
7. **TestFlight** — EN CURSO (2026-07-12). Plan: subir el build **actual** (sin SiwA ni suscripciones) para probar gratis; SiwA + suscripciones van al build final de tienda. Estado: `npm run build && npx cap sync ios` HECHOS; Xcode abierto. **Falta:** en Xcode elegir destino "Any iOS Device (arm64)", verificar Signing (Team 44K4VJQ3Z4, automatic), **Product → Archive**, y subir vía Organizer a App Store Connect. Luego aparece en pestaña TestFlight. (Lo que el usuario "siempre olvida".)
